...
class GoodsController extends BaseController {
    ...
    public function changeTableVal1(){  
        $table = I('table'); // 表名
        $id_name = I('id_name'); // 表主键id名
        $id_value = I('id_value'); // 表主键id值
        $field  = I('field'); // 修改哪个字段
    ...
    public function ajaxGoodsList(){            
        ...
        */
        ...
        foreach ($goodsList as $key => $value) {
            $bbmap['goods_id']=$value['goods_id'];
            $spec_goods_price=D("spec_goods_price")->where($bbmap)->find();
            $goodsList[$key]['sku']=$spec_goods_price['sku'];
            $goodsList[$key]['store_count']=$spec_goods_price['store_count'];
            
            // 获取库存
            // $mapp['good_id'] = $value['goods_id'];
            // $mapp['stock_type'] = 1;
            // $total_number = M('stock')->where($mapp)->sum('number');
            // $goodsList[$key]['store_count'] = $total_number;
        }
            ...
            if (!$Goods->create(NULL, $type))// 根据表单提交的POST数据创建数据对象
            {
                ...
                foreach ($items as $key => $value) {
                    ...
                    if($goods_id){
                        ...
                        $mapsss['sku']=$value['sku'];
                        $lists=D("spec_goods_price")->where($mapsss)->find();

                        if($list){

                            if($lists['goods_id']!=$goods_id){
