...
class SaleWaterController extends BaseController {

	public function index(){
		//权限管理  店长只看门店
        if (session('role_id')>=4) {
            $maps['tp_stock.resource_id'] = array('in',session('store_id'));
            $this->assign('role_id', session('role_id'));
        }
		
		// 搜索条件   门店
		if(!empty(I('store_id'))){
            $maps['tp_stock.resource_id'] = I('store_id');
		...
		$count = M('stock')->join('tp_goods on tp_stock.good_id = tp_goods.goods_id', 'left')->join('tp_spec_goods_price on tp_stock.good_id = tp_spec_goods_price.goods_id', 'left')->join('tp_store on tp_stock.resource_id = tp_store.store_id', 'left')->where($maps)->count();
		...
		foreach ($result as $key => $val) {
			...
			// 计算销售金额
			$amount['tp_order.pay_status'] = 1;
			$amount['tp_order.order_status'] = 4;
			$amount['tp_order.store_id'] = $val['resource_id'];
			$amount['tp_order.add_time'] = array(array('gt',$start_time),array('lt',$end_time));
			$amount['tp_order_goods.goods_id'] = $val['good_id'];
			$order_idArr = M('order')->join('tp_order_goods ON tp_order.order_id=tp_order_goods.order_id','left')->where($amount)->select();
			$total_amount = 0;
			foreach ($order_idArr as $key1 => $value111) {
				$total_amount += $value111['goods_price']*$value111['goods_num'];
			}
    ...
    {	
        ...
        if(!empty($_POST['store_id'])){
            $maps['tp_stock.resource_id'] = $_POST['store_id'];
        }
        // 搜索条件   商品编号
		if(!empty($_POST['sku'])){
    	...
    	$title = array('编号', '商品编号', '商品名称', '单位', '门店名称', '期初', '进货', '销售数', '销售额', '返货', '期末');
    	$name = "saleWater".date("Y/m/d");
    ...
    {	
    	$result = M('stock')->join('tp_goods on tp_stock.good_id = tp_goods.goods_id', 'left')->join('tp_spec_goods_price on tp_stock.good_id = tp_spec_goods_price.goods_id', 'left')->join('tp_store on tp_stock.resource_id = tp_store.store_id', 'left')->where($maps)->field('tp_stock.*, tp_goods.goods_name, tp_goods.spu, tp_goods.market_price, tp_spec_goods_price.sku, tp_store.store_name')->order('good_id asc, resource_id desc')->select();
