...
class ArticleController extends BaseController {
    ...
    public function articleList(){
        ...
        $cat_id = I('cat_id',0);
        if($cat_id==0){
             $where.=" and cat_id in (1,2,3) ";
        }else{
             $cat_id && $where.=" and cat_id = $cat_id ";
        }
        $res = $Article->where($where)->order('orderby desc,article_id desc')->page("$p,$pageCount")->select();
        //echo M()->getlastsql();
        $count = $Article->where($where)->count();// 查询满足要求的总记录数
        $pager = new \Think\Page($count,$pageCount);// 实例化分页类 传入总记录数和每页显示的记录数
        $page = $pager->show();//分页显示输出

        $ArticleCat = new ArticleCatLogic();
        $cats = $ArticleCat->article_cat_list(0,0,false,2);
        if($res){
        	foreach ($res as $val){
     ...
     public function app_articleList(){
        $Article =  M('app_article'); 
        $res = $list = array();
        $p = empty($_REQUEST['p']) ? 1 : $_REQUEST['p'];
        // $size = empty($_REQUEST['size']) ? 20 : $_REQUEST['size'];
        $pageCount = $_GET['pageCount'];
     ...
     public function app_article(){
        ...
        if(I('GET.article_id')){
           ...
           $info = D('app_article')->where('article_id='.$article_id)->find();
        }else{

              $orderby = D('app_article')->max('orderby')+1;
              $info['orderby']=$orderby;
        }
        $cats = $ArticleCat->article_cat_list(0,$info['cat_id'],true,2);
       // dump($cats );
    ...
    public function article(){
        ...
        if(I('GET.article_id')){
           ...
           $info = D('article')->where('article_id='.$article_id)->find();
              ...
              $orderby = D('article')->max('orderby')+1;
              $info['orderby']=$orderby;
        }
        $cats = $ArticleCat->article_cat_list(0,$info['cat_id'],true,2);
       // dump($cats );
    ...
    public function news(){
        ...
        $cats = $ArticleCat->article_cat_list(0,$info['cat_id'],true,1);
       // dump($cats );
        $this->assign('cat_select',$cats);
        $this->assign('act',$act);
        $this->assign('info',$info);
    ...
    public function app_articleHandle(){
        $data = I('post.');
        $data['publish_time'] = strtotime($data['publish_time']);
        //$data['content'] = htmlspecialchars(stripslashes($_POST['content']));
        if($data['act'] == 'add'){
                $data['click'] = mt_rand(1000,1300);
            $data['add_time'] = time(); 
            $r = D('app_article')->add($data);
        }
        
        if($data['act'] == 'edit'){
            $data['updatetime'] =time();
            $r = D('app_article')->where('article_id='.$data['article_id'])->save($data);
        }
        
        if($data['act'] == 'del'){
            $r = D('app_article')->where('article_id='.$data['article_id'])->delete();
            if($r) exit(json_encode(1));        
        }
        $referurl = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : U('Admin/Article/articleList');
        if($r){
    ...
    public function linkList(){
    	...
    	$res = $Ad->where('1=1')->order('orderby')->page($_GET['p'],$pageCount)->select();
    	if($res){
    		foreach ($res as $val){
    ...
    public function changeTop(){
         ...
         $where['cat_id']=$_REQUEST['cate_id'];
         $data['is_top']=0;
         $data['update_time']=time();
         $res=D("Article")->where($where)->save($data);
         if($res){
             $where1['article_id']=$_REQUEST['id'];

              $data['is_top']=1;
              $res=D("Article")->where($where1)->save($data);
         }
    }
    ...
    public function help_list(){
        ...
        $ArticleCat = new ArticleCatLogic();
        $cats = $ArticleCat->article_cat_list(0,0,false,1);
        if($res){
            foreach ($res as $val){
                $val['category'] = $cats[$val['cat_id']]['cat_name'];
                $val['add_time'] = date('Y-m-d H:i:s',$val['add_time']);                
    ...
    public function ajax_share(){
        ...
        $count = M('share')->where($condition)->count();
        ...
        $show = $Page->show();
        $info = M('share')->where($condition)->limit($Page->firstRow,$Page->listRows)->order("id desc")->select();
        $this->assign('info',$info);
    ...
    public function share(){
        $maps['id'] = $_REQUEST['id'];
        $map['pid'] = $_REQUEST['id'];
        $info = D('share')->where($maps)->find();
        $infos = D('share_pic')->where($map)->select();
        $r['user_id'] = $info['user_id'];
        $user = D('users')->where($r)->find();
    ...
    public function del_share(){
        $maps['id'] = $_REQUEST['id'];
        $map['pid'] = $_REQUEST['id'];
        $info = D('share')->where($maps)->delete();
        $infos = D('share_pic')->where($map)->delete();
