...
class SystemController extends BaseController{
	...
	{          
		...
		$group_list = array('shop_info'=>'网站信息','basic'=>'基本设置');			
     ...
     */        
     ...
     public function edit_right(){
     	...
     	$group = array('system'=>'系统设置','content'=>'内容管理','goods'=>'商品中心','member'=>'会员中心','service'=>'会员服务',
     			'order'=>'订单中心','marketing'=>'营销推广','tools'=>'插件工具','count'=>'统计报表','admin'=>'管理员管理','inventory'=>'进销存管理','wechat'=>'微信管理'
     	);
     ...
     public function ajax_area(){
        ...
        $count = M('trading_area')->where($condition)->count();
        ...
        $show = $Page->show();
        $info = M('trading_area')->where($condition)->limit($Page->firstRow,$Page->listRows)->order("id desc")->select();
        $this->assign('info',$info);
        $this->assign('page',$show);
        $this->display();
     }
     public function shangquang(){
        $maps['id'] = $_REQUEST['id'];
        $data['status'] = 0;
        $info = D('trading_area')->where($maps)->save($data);
        if($info) {
     ...
     public function area(){
        ...
        $data['status'] = 1;
        $add = D('trading_area')->add($data);
        if($add){
            $this->success("新增成功",U('admin/System/trading_area'));
        }
     }
}