...
class OrderLogic extends RelationModel
{
    ...
    public function orderProcessHandle($order_id,$act,$del_reason=''){
    	$updata = array();
    	switch ($act){
    		case 'pay': //付款
    		...
    		case 'invalid': //作废订单
                ...
                //添加删除原因
                $updata['del_reason'] = $del_reason;

                $massp['order_id']=$order_id;
                $account_log=D("account_log")->where($massp)->find();

                //订单生成新增积分
                $addpoint['order_id']=$order_id;
                $addpoint['type']=2;
                $add_pay_points=D("account_log")->where($addpoint)->getField('pay_points');
                
                //订单生成减少积分(使用积分支付了部分金额)
                $addpoint1['order_id']=$order_id;
                $addpoint1['type']=3;
                $reduce_pay_points=D("account_log")->where($addpoint1)->getField('pay_points');

                $order=D("order")->where($massp)->find();

                $pay_points=$account_log['pay_points'];
                $order_amount=$order['order_amount'];
                $user_id=$order['user_id'];

                $mapssss['user_id']=$user_id;

                $user=D("Users")->where($mapssss)->find();
                $mapss['user_id']=$user_id;

                // //如果存在新增积分 则返回
                // if ($add_pay_points) {
                //     $nowPoints=$user['pay_points']-$add_pay_points;
                // }else{
                //     $nowPoints=$user['pay_points'];
                // }
                // //如果存在减少积分 则返回
                // if ($reduce_pay_points) {
                //     $nowPoints=$nowPoints+$reduce_pay_points;
                // }
                $nowPoints=$add_pay_points-$reduce_pay_points;
                $data['total_amount']=$user['total_amount']-$order_amount;
                if ($data['total_amount']<0) {
                    $data['total_amount'] = 0;
                }
                // $data['pay_points']=$nowPoints;
                adminLog('删除订单编号为'.$order['order_sn'].'的订单');//操作日志
                $_thisRe = D("Users")->where($mapss)->save($data);
                if($account_log){
                    //返回积分
                    accountLog($user_id,0,$nowPoints,'作废订单返分',0,3,1,$order_id);
                }
                if ($order['order_status']==4) {
                    //返回商品
                    return_goods($user_id,0,$pay_points,'作废订单退回商品',0,0,1,$order_id,$order['store_id']);
                }
                 //echo M()->getlastsql();
