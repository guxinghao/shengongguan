 ...
 * @return  bool
 */
function accountLog($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0,$type,$scoretype){
    /* 插入帐户变动记录 */
...
//获取可用积分
 function getKeScore($user_id){
    ...
    $time2=strtotime(date('Y-m-d'))+3600*24-1;
    $map['user_id']=$user_id;
    $user=D("Users")->where($map)->find();
    $map['type']=3;
    $map['change_time']=array(array('gt',$time1),array('lt',$time2));
    //$score1=D("account_log")->where($map)->sum("pay_points");
    $map['type']=array('neq','3');
    $score2=D("account_log")->where($map)->sum("pay_points");

    $score3=$user['pay_points']-$score2+$score1;
...
/*function getTypes($uid,$total_amount,$type)
{
    ...
    if($total_amount>=20000){
       ...
       // exit;
        ...
        $yeartime1=time()-365*24*3600;

        $total_amount = M('order')->where("user_id=$uid AND pay_status=1 and  $yeartime1<add_time and order_status not in (3,5)")->sum('total_amount');
        ...
        if($user['fengge_id']==7||$user['fengge_id']==8){
         ...
         $count = M('order')->where("user_id=$uid AND pay_status=1 and  $yeartime1<add_time  and order_status not in (3,5)")->count();

         if($count>=3){

              if($user['fengge_id']<5&&$user['fengge_id']>6){
                $datas['fengge_id']=5;
                $datas['user_id']=$uid;
                $res=D("users")->save($datas);
                if($res){
         ...
         if($count>=2){
             ...
             if($user['fengge_id']<4&&$user['fengge_id']>6){
                $datas['fengge_id']=5;
                $datas['user_id']=$uid;
                $res=D("users")->save($datas);
                if($res){
