...
//判断某串字符是否存在某个指定的值  
function getStrParam (name,str) {
    var r = str.indexOf(name);
    if (r != -1) return true; return false;
}  

//替换最后一个'/'之后的所有内容  
function reStrParam (str,name) {
    // 字符串长度
    var m = str.length;
    //最后一个'/'的位置
    var n = str.lastIndexOf('/')+1;
    //获取指定位置内的字符串
    var _str = str.substring(n,m);
    //替换指定的内容
    var s = str.replace(_str,name);
    return s;
}           