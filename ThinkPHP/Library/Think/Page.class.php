...
//                    $link_page .= '<span class="current">' . $page . '</span>';
        ...
        $page_str = str_replace(
            ...
            $this->config['theme']);
        if (preg_replace('# #', '', $page_str)) {
            return "<div class='dataTables_paginate paging_simple_numbers'><ul class='pagination'>{$page_str}<select style='margin-left:5px;height:32px;border-radius:5px;' id='selectPage' name='page'><option ".($this->listRows==25?'selected="selected"':'')." value='25'>25</option><option ".($this->listRows==50?'selected="selected"':'')." value='50'>50</option><option ".($this->listRows==100?'selected="selected"':'')." value='100'>100</option></select></ul></div><script>$('.pagination li').eq(-1).find('a').css('border-top-right-radius','4px');
    $('.pagination li').eq(-1).find('a').css('border-bottom-right-radius','4px');</script>";
        }
